document.getElementById('start-button').addEventListener('click', function() {
    ...
    fetchUserData()
        ...
        .then(posts => {
            ...
            document.getElementById('output').innerHTML += `<p>User posts: ${posts.join(', ')}</p>`;    
        ...
        .then(comments => {
            ...
            document.getElementById('output').innerHTML += `<p>Comments for first post: ${comments.join(', ')}</p>`;
...
function fetchComments(postTitle) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            // Simulating a successful data fetch
            resolve(["Comment1", "Comment 2", "Comment 3"]);
        }, 2000);
    });
